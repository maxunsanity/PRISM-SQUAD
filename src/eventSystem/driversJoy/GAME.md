...
identity:
  name: "드라이버의 기쁨"
  name_en: "Driver's Joy"
  genre: [liveops, iap_offer, popup_sale]
  reference: "모노폴리 GO — 드라이버의 기쁨 (단일 패키지 + 원화 결제 + 구매 횟수 제한)"
  module_path: "src/eventSystem/driversJoy/"
  data_path: "public/event/driversJoy/"
  dev_entry: "DEV.md"
  host_coupling: "느슨 — SalesHostBridge"
  ui_slot:
    lobby_side_tab: "화면 왼쪽 (쇼핑몰 탭 아래)"
    modal: "전투 로비 오버레이 z62"
  implementation_status: "MVP 구현 완료 (CSV · localStorage · 테스트 캐시)"
  registry_key: "drivers_joy"
  tycoon_mileage_reuse: "없음"

event_type: IAP_PACKAGE

components:
  phase:
    type: enum
    values: [INACTIVE, ACTIVE, SOLD_OUT, EXPIRED]
  purchases_used:
    type: int
    range: [0, max_purchase_per_player]
    persist: "localStorage prism_dj_purchases_v1"
  timer_ends_at:
    type: timestamp_ms
    persist: "localStorage prism_dj_ends_at_v1"

entities:
  offer_pack:
    desc: "단일 번들 1종 (1차)"
    components: [price_krw, max_purchase, reward_lines]

mechanics:
  purchase:
    preconditions:
      - "now < timer_ends_at"
      - "purchases_used < max_purchase"
      - "host.cashKrw >= price_krw"
    effects:
      - "cash 차감"
      - "dj_reward_config 전 행 지급"
      - "purchases_used += 1"

design_rules:
  - "구매 N/M UI는 모노 GO 연출 (1차: localStorage, 서버 계정 X)"
  - "결제 1차: metaCashKrw"
  - "보상·가격·횟수는 CSV만"
  - "tycoon EventBridge / eventStore 공유 금지"
---
