---
identity:
  name: "쇼핑몰의 경이로움 — DESIGN"
  name_en: "Mall Marvels — UI/UX Design Spec"
  module_path: "src/eventSystem/mallMarvels/"
  reproducibility: "재현 가능 등급 — 실제 컴포넌트명·z-index·상태 경로·스타일값 보존"
  source_of_truth:
    - "src/eventSystem/sales/SalesEventOverlay.tsx (왼쪽 탭 셸)"
    - "src/eventSystem/mallMarvels/jsonRender/MallMarvelsModal.tsx (모달)"
    - "src/eventSystem/tycoonSeason/jsonRender/eventHudLayout.ts (탭 셸 스타일)"
    - "src/App.tsx (z-index 레이어)"
  note: "픽셀 단위 비주얼 폴리시는 외부 팀. 본 문서는 구현된 실제 DOM·스타일을 재현 가능하게 고정."
---
